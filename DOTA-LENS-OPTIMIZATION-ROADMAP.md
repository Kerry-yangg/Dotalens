...
> 当前实现基线：桌面端 `0.4.0`、本地 API `1.1.0`、分析包 `dota-lens/0.9`、产品模块 `product-modules/2.4`  
...
当前阶段暂停 30 场双人标注、仲裁和所有依赖人工黄金样本的生产任务，保留现有页面、接口和已保存数据，但不把它们放在版本关键路径上。
...
当前模型使用固定线性权重组合补刀、反补、等级、经验、死亡和辅助路线，尚未经过黄金样本校准。
...
建立至少 30 场人工标注比赛，覆盖线上消耗、2v2、2v3、3v3、5v5、无死亡逼退、长追击、双地点同时冲突、肉山、高地、幻象和买活。
...
**实施状态（0.3.0）：基础设施保留，人工生产与标注相似度覆盖默认分类均暂停。**

- [x] A/B 独立盲标、仲裁三个隔离存储槽。
- [x] 真实小地图、英雄位置、毫秒播放头、原始事件窗口和全场事件密度定位。
- [x] 分类、四段时间边界、参与者、标签、圆心、半径、置信度和备注字段。
- [x] 完成状态硬校验：最多回溯 10 秒、2v2 禁止团战、团战至少 5 人且双方至少 2 人。
- [x] 单场一对一匹配评测和仅统计仲裁完成文件的全库发布门槛。
- [x] 仲裁差异总览以及从 A/B 导入仲裁底稿。
- [x] 冻结标签可进入跨场特征库；默认产品分析继续使用自动事实与规则模型，不依赖这些标签。
- [x] 排除当前比赛样本的近邻预测、按比赛留一验证、规则基线和相似样本证据回溯。
- [x] 标注时间统一使用与游戏一致的 `分:秒.毫秒` 显示和输入。
- [ ] 选取并完成 30 场双人标注与仲裁。
- [ ] 基于首批分歧复核操作定义，并冻结标注规范 1.0。

详细操作口径见 `COMBAT-GOLDEN-ANNOTATION-GUIDE.md`。
...
- 至少标注 40 个 10 分钟三路线况、20 个越塔片段、20 个肉山生命周期和 50 个逐玩家建议窗口。
- 越塔事件召回率不低于 95%，越塔 + TP 进入关键战斗召回率不低于 98%。
- TP 开始/完成/打断识别 F1 不低于 0.95。
- 肉山击杀、盾拾取和盾结束时间准确率为 100%；有争夺肉山事件召回率不低于 95%。
- 所有主要玩家建议必须存在可点击证据；证据不足仍输出确定责任判断必须为 0 次。
...
不再把人工标注生产作为当前阻塞项。下一阶段优先完成 `DATA-03` 的 patch 化塔、营地、神符、肉山坑和区域校准，并修复当前 870 条无效坐标导致的 `partial` 完整性；同时为战斗聚类加入交战空窗、中心跃迁和目标切换切分，收敛 79 至 123 秒长片段。随后接入 `COMBAT-08` 的战前真实视野证据，并为 `COMBAT-12` 补齐库存、冷却、魔法、死亡状态和可挽救性门禁。肉山/盾需扩展到至少 30 场多版本样本，覆盖抢肉、抢盾和买活后争夺。每一步继续使用程序不变量、重跑一致性、阈值稳定性和真实多场分布对比验收，不输出未经人工真值验证的精确率数字。
...
|---|---|---|---|
| 多 Patch 地图校准 | 第一版完成 | 7.33 前、7.33 至 7.40、7.41 三套 profile；Replay 营地与建筑动态锚点；未知版本显式回退 | 静态 profile 的 P95 世界坐标误差仍需多版本黄金样本 |
