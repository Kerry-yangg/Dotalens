...
> 审计日期：2026-07-26；最近回填：2026-07-28
> 审计对象：本地工作区、历史产品 PRD、专题规则文档、实施计划、发行报告和当前自动化测试  
> 本地分支：`codex/player-report-atomic-matrix`
> 当前源码版本：桌面端 `0.5.0`、本地 API `1.7.0`
> 当前报告协议：`player-report/4.0`
...
0.4.5 基线和 0.5.0 玩家报告开发验收均已通过，但按当前决定没有发布 0.5.0 到 Git。更准确的阶段判断是：

| 层级 | 当前判断 |
| --- | --- |
| 公开发行版 | GitHub 公开主线和标签仍停在 `0.4.2` |
| 本地安装包 | `Dota-Lens-Setup-0.5.0-x64.exe` 已完成全流程验收，未发布 |
| 当前源码 | `0.5.0`、Parser API `1.7.0`，保留在独立开发分支 |
| 当前开发内容 | `player-report/4.0`、统一事件候选池、简明/专业双层报告、精确跳转和双层 Replay 回归 |
| 0.5.0 玩家报告 | 15/15 Replay、150/150 全员协议、15/15 Golden、四档视口和 EXE 冒烟已通过 |
...
1. 保留已通过的 `0.5.0` 本地工程与安装包，Git 发布继续延后。
2. 直接进入 `0.5.1` JSON、首屏、任务与桌面可靠性。
3. 不继续扩张玩家报告页面结构，只修真实回归发现的问题。
...
> 本节保存 2026-07-26 的历史排序；当前排序以第 21 节为准。
...
> 本节保存 2026-07-26 的历史决策，0.4.5 和 0.5.0 对应门槛均已完成；当前任务以第 21 节为准。
...
## 21. 2026-07-28 最新阶段判断与开发顺序

### 21.1 已完成

- `player-report/4.0` 可重算计分、三路计分、去重和单根因封顶。
- 原子基础组件和前端独立审计链路。
- 统一事件候选池，覆盖眼位、装备、阵亡、目标、TP 和既有洞察。
- 全场重要时刻选择、聚类去重、简明/专业双层展示。
- 优缺点时间、地图、实体、模块、有界播放和返回上下文。
- 15 场、150 份协议、15 份 Golden 双层矩阵。
- 四档玩家报告窗口和五个附加页面布局验收。
- 0.5.0 安装、启动、取消、完整解析、冷重启、持久化和卸载冒烟。

### 21.2 当前 P0

下一个实际开发任务是：

> **0.5.1 JSON 体积与首屏性能第二阶段。**

实施顺序：

1. farm 按玩家、时间窗和数据类型裁剪的轻量接口。
2. 摘要可见、图表可用、玩家报告可读三段真实性能预算。
3. 拆分主入口和图表运行时两个大于 500 kB 的分包。
4. 冷解析阶段计时、P50/P95 基线和最大阶段定位。
5. Parser 中心任务历史与渲染、磁盘、权限、日志诊断。
