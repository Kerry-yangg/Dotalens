...
这些入口只能提高召回，不能绕过第 7.6 节团战硬门槛。候选缺少双方有效交互时可以最终归类为 `non_combat/objective_attempt`，并保留未成立理由供规则诊断和回归测试。
...
维护覆盖线上换血、2v2、多人交战、长追击、双地点冲突、肉山、塔下、高地、幻象、买活、TP 支援和不同 Patch 的固定 Replay 集。每场只保存可由程序重复核验的事实断言、规则不变量和输出快照，不在产品中维护人工标注、仲裁或跨场标签学习。

回归集必须检查同一 Replay 重跑一致性、2v2 不升级团战、时间排序、参与者证据、坐标有效性、缺失数据降级和阈值扰动稳定性。语义精确率、召回率及 F1 没有独立外部真值时不得作为已达成指标。
...
| 有争夺肉山事件召回率 | 不低于 95% |
| 无争夺打肉误报肉山团 | 不高于 2% |
| 比赛阶段 30 秒窗口重跑一致率 | 100% |
| 主要玩家建议具备可点击证据 | 100% |
| 证据不足仍输出确定性责任判断 | 0 次 |

指标门槛为评审默认值；精确率、召回率和 F1 只有接入独立外部真值后才可正式验收。
...
4. 使用固定回放回归集调参并冻结 `combat-rules/1.0`。
5. 实现与战斗类型正交的关键等级，以及塔下/TP/肉山上下文状态机。
6. 使用跨比赛无监督分布审计发现异常，同时保留硬门槛和规则基线。
