...
> 当前约束：暂停依赖人工标注的功能与验收，不删除既有标注数据和接口
...
3. 人工标签暂停后，无法诚实计算语义分类的精确率和召回率；模型置信度只能称为“规则/模型置信度”，不能冒充已校准概率。
...
语义准确率仍需要未来的小规模抽查或黄金样本才能最终确认，这是无法通过无监督学习绕开的边界。
...
5. `QA-01` 人工标注与黄金样本验收进入暂停状态，不再阻塞自动功能开发。
